...
def setup_repos():
    if not os.path.isdir('gitian.sigs'):
        subprocess.check_call(['git', 'clone', 'https://github.com/1x2coin-Project/gitian.sigs'])
    if not os.path.isdir('1x2coin-detached-sigs'):
        subprocess.check_call(['git', 'clone', 'https://github.com/1x2coin-Project/1x2coin-detached-sigs'])
    if not os.path.isdir('gitian-builder'):
        subprocess.check_call(['git', 'clone', 'https://github.com/devrandom/gitian-builder.git'])
    if not os.path.isdir('1x2coin'):
        subprocess.check_call(['git', 'clone', 'https://github.com/1x2-coin/1x2coin.git'])
...
def build():
    ...
    os.makedirs('1x2coin-binaries/' + args.version, exist_ok=True)
    ...
    subprocess.check_call(['make', '-C', '../1x2coin/depends', 'download', 'SOURCES_PATH=' + os.getcwd() + '/cache/common'])

    if args.linux:
        print('\nCompiling ' + args.version + ' Linux')
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', '1x2coin='+args.commit, '--url', '1x2coin='+args.url, '../1x2coin/contrib/gitian-descriptors/gitian-linux.yml'])
        subprocess.check_call(['bin/gsign', '-p', args.sign_prog, '--signer', args.signer, '--release', args.version+'-linux', '--destination', '../gitian.sigs/', '../1x2coin/contrib/gitian-descriptors/gitian-linux.yml'])
    ...
    if args.windows:
        ...
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', '1x2coin='+args.commit, '--url', '1x2coin='+args.url, '../1x2coin/contrib/gitian-descriptors/gitian-win.yml'])
        subprocess.check_call(['bin/gsign', '-p', args.sign_prog, '--signer', args.signer, '--release', args.version+'-win-unsigned', '--destination', '../gitian.sigs/', '../1x2coin/contrib/gitian-descriptors/gitian-win.yml'])
        subprocess.check_call('mv build/out/1x2coin-*-win-unsigned.tar.gz inputs/', shell=True)
        subprocess.check_call('mv build/out/1x2coin-*.zip build/out/1x2coin-*.exe build/out/src/1x2coin-*.tar.gz ../1x2coin-binaries/'+args.version, shell=True)

    if args.macos:
        print('\nCompiling ' + args.version + ' MacOS')
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', '1x2coin='+args.commit, '--url', '1x2coin='+args.url, '../1x2coin/contrib/gitian-descriptors/gitian-osx.yml'])
        subprocess.check_call(['bin/gsign', '-p', args.sign_prog, '--signer', args.signer, '--release', args.version+'-osx-unsigned', '--destination', '../gitian.sigs/', '../1x2coin/contrib/gitian-descriptors/gitian-osx.yml'])
        subprocess.check_call('mv build/out/1x2coin-*-osx-unsigned.tar.gz inputs/', shell=True)
        subprocess.check_call('mv build/out/1x2coin-*.tar.gz build/out/1x2coin-*.dmg build/out/src/1x2coin-*.tar.gz ../1x2coin-binaries/'+args.version, shell=True)
...
def sign():
    ...
    subprocess.check_call('cp inputs/1x2coin-' + args.version + '-osx-unsigned.tar.gz inputs/1x2coin-osx-unsigned.tar.gz', shell=True)
    subprocess.check_call(['bin/gbuild', '--skip-image', '--upgrade', '--commit', 'signature='+args.commit, '../1x2coin/contrib/gitian-descriptors/gitian-osx-signer.yml'])
    subprocess.check_call(['bin/gsign', '-p', args.sign_prog, '--signer', args.signer, '--release', args.version+'-osx-signed', '--destination', '../gitian.sigs/', '../1x2coin/contrib/gitian-descriptors/gitian-osx-signer.yml'])
    subprocess.check_call('mv build/out/1x2coin-osx-signed.dmg ../1x2coin-binaries/'+args.version+'/1x2coin-'+args.version+'-osx.dmg', shell=True)
    ...
    if args.commit_files:
        os.chdir('gitian.sigs')
        commit = False
        if os.path.isfile(args.version+'-win-signed/'+args.signer+'/1x2coin-win-signer-build.assert.sig'):
            subprocess.check_call(['git', 'add', args.version+'-win-signed/'+args.signer])
            commit = True
        if os.path.isfile(args.version+'-osx-signed/'+args.signer+'/1x2coin-dmg-signer-build.assert.sig'):
...
def verify():
    ...
    if subprocess.call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-linux', '../1x2coin/contrib/gitian-descriptors/gitian-linux.yml']):
        print('Verifying v'+args.version+' Linux FAILED\n')
        rc = 1

    print('\nVerifying v'+args.version+' Windows\n')
    if subprocess.call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-win-unsigned', '../1x2coin/contrib/gitian-descriptors/gitian-win.yml']):
        print('Verifying v'+args.version+' Windows FAILED\n')
        rc = 1

    print('\nVerifying v'+args.version+' MacOS\n')
    if subprocess.call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-osx-unsigned', '../1x2coin/contrib/gitian-descriptors/gitian-osx.yml']):
        print('Verifying v'+args.version+' MacOS FAILED\n')
        rc = 1

    # TODO: Skip checking signed windows sigs until we actually start producing signed windows binaries
    #print('\nVerifying v'+args.version+' Signed Windows\n')
    #if subprocess.call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-win-signed', '../1x2coin/contrib/gitian-descriptors/gitian-win-signer.yml']):
    #    print('Verifying v'+args.version+' Signed Windows FAILED\n')
    #    rc = 1

    print('\nVerifying v'+args.version+' Signed MacOS\n')
    if subprocess.call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-osx-signed', '../1x2coin/contrib/gitian-descriptors/gitian-osx-signer.yml']):
...
def main():
    ...
    if args.pull:
        ...
        if not os.path.isdir('../gitian-builder/inputs/1x2coin'):
            os.makedirs('../gitian-builder/inputs/1x2coin')
        os.chdir('../gitian-builder/inputs/1x2coin')
